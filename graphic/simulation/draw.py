...
from geom2d import AffineTransform, Segment, Polygon, Circle, Rect
...
class CanvasDrawing:
    ...
    def draw_circle(self, circle: Circle, division=30):
        self.__draw_polygon(
            self.transform.apply_to_circle(circle, division)
        )
    ...
    def draw_arrow(
        ...
        self.draw_segment(
            Segment(
                segment.end,
                segment.end.displaced(v_1 + v_h1)
            )
        )
        self.draw_segment(
            Segment(
                segment.end,
                segment.end.displaced(v_1 + v_h2)
            )
        )
