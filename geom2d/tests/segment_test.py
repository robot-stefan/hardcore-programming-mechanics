...
class TestSegment(unittest.TestCase):
    ...
    def test_parallel_segment_no_intersection(self):
        ...
        actual = self.segment.intersect_with(other)
    ...
    def test_segments_intersection_out1(self):
        # Testing intersection point is only on self
        other = Segment(Point(0, 0), Point(-200, -200))
        actual = self.segment.intersect_with(other)
        self.assertIsNone(actual)

    def test_segments_intersection_out2(self):
        # Testing intersection point is only on other
        other = Segment(Point(-100, 0), Point(-100, 1000))
        actual = self.segment.intersect_with(other)
        self.assertIsNone(actual)

    def test_segments_intersection_out3(self):
        # Testing intersection point is outside of both segments
        other = Segment(Point(0, 0), Point(400, -200))
        actual = self.segment.intersect_with(other)
        self.assertIsNone(actual)
